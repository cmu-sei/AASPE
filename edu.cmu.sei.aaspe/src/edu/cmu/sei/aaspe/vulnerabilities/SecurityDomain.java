...
public class SecurityDomain extends AbstractVulnerability { 
	@Override
	public List<Vulnerability> findVulnerabilities(ComponentInstance component) {
			...
			for (ConnectionInstance ci : fi.getDstConnectionInstances())
			{
				if (ci.getKind() == ConnectionKind.PORT_CONNECTION)
				{
					...
					if ( (PropertyUtils.isVerified(componentDestination) == false) &&
							(edu.cmu.sei.aaspe.utils.Utils.equalsSecurityLevelsOrDomain(ComponentUtils.getSecurityDomains (source), ComponentUtils.getSecurityDomains (destination)))) {
						Vulnerability v = new Vulnerability();
						v.setType(Vulnerability.VULNERABILITY_SECURITY_DOMAIN);
						v.setName ("Security Domains");
						v.setComment("component " + component.getName() + " and " + componentDestination.getName() + " are connected and do not share the same security domains");
		...
		if ((processor != null ) && (processor.getCategory() == ComponentCategory.PROCESSOR))
		{
			for (ComponentInstance ci : ComponentUtils.getBoundProcessesToProcessor (processor))
			{
				...
				if ( (ci.getCategory() == component.getCategory()) &&
						(edu.cmu.sei.aaspe.utils.Utils.equalsSecurityLevelsOrDomain (ComponentUtils.getSecurityDomains (ci), ComponentUtils.getSecurityDomains (component)) == false))
				{
					Vulnerability vulnerability = new Vulnerability();
					vulnerability.setType(Vulnerability.VULNERABILITY_SECURITY_DOMAIN);
					vulnerability.setName ("Security Domains");
					vulnerability.setComment("component " + component.getName() + " and " + ci.getName() + " are on the same processor and does not share the same security domains");
					vulnerability.setRelatedElement(component);
					vulnerability.addPropagations(PropagationModel.getInstance().findPropagations(component));		
		...
		for (ComponentInstance memory : memories)
		{
			for (ComponentInstance ci : ComponentUtils.getBoundComponentsToMemory (memory))
			{
					...
					(edu.cmu.sei.aaspe.utils.Utils.equalsSecurityLevelsOrDomain (ComponentUtils.getSecurityDomains (ci), ComponentUtils.getSecurityDomains(component)) == false))
				{
					...
					vulnerability.setType(Vulnerability.VULNERABILITY_SECURITY_DOMAIN);
					vulnerability.setName ("Security Domains");
					vulnerability.setComment("component " + component.getName() + " and " + ci.getName() + " share the same memory and does not share the same security domains");
					vulnerability.setRelatedElement(component);
					vulnerability.addPropagations(PropagationModel.getInstance().findPropagations(component));		
